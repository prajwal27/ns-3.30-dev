...
sudo ip link set eth4 netns ns3
...
sudo ip netns exec ns3 ip link set eth4 up
...
sudo ip netns exec ns3 ip address add 10.0.4.1/24 dev eth4
...
sudo ip netns exec ns3 ip route add default via 10.0.4.2 dev eth4

#Enable IP forwarding : Make it act as a router
sudo ip netns exec router sysctl -w net.ipv4.ip_forward=1
#Pinging the interface 
sudo ip netns exec ns2 ping -c 1 10.0.0.1
sudo ip netns exec ns3 ping -c 2 10.0.0.1
