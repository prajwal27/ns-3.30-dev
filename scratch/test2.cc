...
main (int argc, char *argv[])
{
  ...
  OnOffHelper server ("ns3::TcpSocketFactory", (InetSocketAddress (interface2.GetAddress (0), port)));
   //server.SetAttribute ("PacketSize", UintegerValue (payloadSize));
   server.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
   server.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
   //server.SetAttribute ("DataRate", DataRateValue (DataRate (dataRate)));
   ApplicationContainer onOff = server.Install (nodes2.Get(1));
   onOff.Start(Seconds(1.0));
   onOff.Stop(Seconds(10.0));
  ...
  ApplicationContainer sinkApps2 = packetSinkHelper2.Install (nodes2.Get (0)); 
